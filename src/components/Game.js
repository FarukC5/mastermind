...
export class Game extends React.Component {
  constructor(props) {
    ...
    const rowColors = [...colors];
    const trueRow = [];

    for (let i = 0; i < 4; i++) {
      const random = Math.floor(Math.random() * rowColors.length);
      trueRow.push(rowColors[random]);
      rowColors.splice(random, 1);
    }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Solution isHidden state={this.state} newGame={this.newGame} />
      </div>
    );
  }
}
